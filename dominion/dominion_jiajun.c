  ...
  for (i = 0; i < state->handCount[player]; i++)
    {
      ...
      if (state->hand[player][i] == gardens) { score += (state->handCount[player] + state->deckCount[player] + state->discardCount[player]); };
    }
  ...
  for (i = 0; i < state->discardCount[player]; i++)
    {
      ...
      if (state->discard[player][i] == gardens) { score += (state->handCount[player] + state->deckCount[player] + state->discardCount[player]); };
    }
  ...
  for (i = 0; i < state->discardCount[player]; i++)
    {
      ...
      if (state->deck[player][i] == gardens)  { score += (state->handCount[player] + state->deckCount[player] + state->discardCount[player]); };
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
 ...
 //     while( x == 1) {//Buy one card
    ...
    case minion:
      ...
      if (choice1)		//+2 coins
	{
//	  state->coins = state->coins + 2;bug found
	  *bonus += 2;// this is correct one
	}
